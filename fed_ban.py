...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**STARTING userbotS BAN BY {PRO}**...")
    fedList = []
    ...
    try:
        ...
        if FBAN == "@PROBOYX" or FBAN == "@userbotX22":
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**STARTING userbotS UNFBANNING BY {PRO}**...")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
...
CMD_HELP.update(
    {
        ...
        \n\nGet help - @userbotX22 OR @PROBOYX."
    }
)
